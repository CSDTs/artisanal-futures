...
const ProductDetails: FC<IProps> = ({ children, ...product }) => {
	...
	const [assessmentData, setAssessmentData] = useState(JSON.parse(product.assessment[0].data).calculation);
							...
							<Transition.Child
								...
								<Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
									...
									<section className="flex mt-5 gap-4">
										{" "}
										<div>
											<p className="text-lg text-gray-500 ">{product.description || "No description available."}</p>
																...
																<tbody>
																	{assessmentReference &&
																		Object.keys(assessmentReference).map((entry) => (
																			<tr key={entry} className="odd:bg-white even:bg-slate-50">
																				<td className="w-2/4">
																					<div className="group relative flex justify-left cursor-default">
																						<p>{assessmentReference[entry].id}</p>
																						<span className="absolute left-10 scale-0 transition-all rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100 whitespace-nowrap">
																							{assessmentReference[entry].group +
																								" -- " +
																								assessmentReference[entry].name}
																						</span>
																					</div>
																				</td>
																				<td>
										...
										<img
											src={product.image}
											alt=""
											className="aspect-square object-contain shadow p-3 w-5/12"
